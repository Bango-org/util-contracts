...
library Fixed192x64Math {
        ...
        returns (uint lower, uint upper)
    {
        ...
        shift -= 64;
        if (shift >= 0) {
            uint256 shiftUnsigned = uint256(shift);

            if (uint256(result) >> (256 - shiftUnsigned) == 0) {
                lower = uint256(result) << shiftUnsigned;
                zpow <<= shiftUnsigned; // This is safe since Solidity 0.8.x checks for overflows.

                if (lower + uint256(zpow) >= lower) {
                    upper = lower + uint256(zpow);
                } else {
                    upper = type(uint256).max; // Use type(uint256).max instead of 2**256 - 1 for clarity.
                }

                return (lower, upper);
            } else {
                return (type(uint256).max, type(uint256).max); // Use type(uint256).max for maximum value.
            }
        } else {
            uint256 shiftAbs = uint256(-shift);

            zpow = (zpow >> shiftAbs) + 1;
            lower = uint256(result) >> shiftAbs;
            upper = lower + uint256(zpow);
